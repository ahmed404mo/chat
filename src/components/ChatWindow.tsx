...
function getConversationName(
  participants: { id: string; userId: string; user: { id: string; name: string; role: string } }[],
  userId: string,
  title: string | null,
) {
...
export default function ChatWindow({ onToggleSidebar }: ChatWindowProps) {
  const { user } = useAuth();
  const {
    conversations, activeConversation, messages, onlineUsers, isManager,
    setActiveConversation, markAsRead, addReaction, removeReaction,
    editMessage, deleteMessage, sendMessage, emitTyping, emitStopTyping,
    typingUsers: contextTypingUsers, loadMoreMessages, hasMoreMessages, loadingMoreMessages,
  } = useChat();
  ...
  useEffect(() => {
    if (activeConversation) loadMoreMessages(activeConversation);
  }, [activeConversation, loadMoreMessages]);

  const conversation = conversations.find((c) => c.id === activeConversation);
  const convMessages = conversation ? messages[conversation.id] || [] : [];
  const otherParticipant = conversation?.participants.find((p) => p.userId !== user!.id) || null;
  const convName = conversation ? getConversationName(conversation.participants, user!.id, conversation.title) : "";
  ...
  const audioRef = useMemo(() => typeof Audio !== "undefined" ? new Audio("/notification.mp3") : null, []);

  useEffect(() => {
    const lastMessage = convMessages[convMessages.length - 1];
    if (lastMessage && lastMessage.senderId !== user?.id) {
      if (document.visibilityState !== "visible") audioRef?.play().catch(() => {});
    }
  }, [convMessages, user?.id, audioRef]);

  const typingList = activeConversation
    ? (contextTypingUsers[activeConversation] || []).map((u) => ({ id: u.userId, name: u.name }))
    : [];

  useEffect(() => {
    if (!activeConversation || convMessages.length === 0) return;
    const hasUnread = convMessages.some(
      (m: any) => m.senderId !== user?.id && (!m.readBy || !m.readBy.some((r: any) => r.userId === user?.id)),
    );
  ...
  const handleSend = useCallback((content: string, files?: UploadedFile[]) => {
    if (activeConversation) {
      if (editingMessage) {
        ...
        sendMessage(activeConversation, content, replyingTo?.id || undefined, files, replyingTo);
      }
      setReplyingTo(null);
    }
  }, [activeConversation, replyingTo, editingMessage, editMessage, sendMessage]);

  const handleSendFile = useCallback((content: string, files: UploadedFile[]) => {
    if (activeConversation) {
      sendMessage(activeConversation, content, replyingTo?.id || undefined, files, replyingTo);
      setReplyingTo(null);
    }
    ...
    ? [
        ...
        ...(contextMenu.message.senderId === user?.id
          ? [
              ...
              { id: "delete", label: "Delete for everyone", danger: true as const, icon: (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="3 6 5 6 21 6" /><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
                </svg>
              ), onClick: () => deleteMessage(contextMenu.message.id, activeConversation!, true) },
              { id: "delete-me", label: "Delete for me", danger: true as const, icon: (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                  <path d="M3 12h18" /><path d="M12 3v18" />
                </svg>
              ), onClick: () => deleteMessage(contextMenu.message.id, activeConversation!, false) },
            ]
          : isManager
            ? [{ id: "delete", label: "Delete for everyone", icon: (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="3 6 5 6 21 6" /><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
                </svg>
              ), danger: true as const, onClick: () => deleteMessage(contextMenu.message.id, activeConversation!, true) }]
            : []),
        { id: "message-info", label: "Message Info", icon: (
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" />
          </svg>
        ), divider: true, onClick: () => handleMessageInfo(contextMenu.message) },
      ]
    : [];
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative overflow-hidden">
        <div className="text-center px-8 relative z-10">
          ...
          >
            ...
            <h2 className="text-xl font-semibold text-[var(--color-text)] mb-2">Company Chat</h2>
  ...
  return (
    <>
      <div className="flex flex-col h-full overflow-hidden">
        {/* Premium Header */}
        <div className="flex items-center justify-between px-4 py-2.5 shrink-0 border-b border-[var(--color-border)]"
          style={{ background: "var(--color-header)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)" }}>
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="relative shrink-0 cursor-pointer group" onClick={() => setShowInfo(true)}>
              <div className="rounded-full overflow-hidden ring-2 ring-[var(--color-border)] group-hover:ring-[var(--color-primary)] transition-all duration-300"
                style={{ width: 40, height: 40 }}>
                {(isGroup && conversation.imageUrl) || (!isGroup && otherParticipant?.user?.avatarUrl) ? (
                  <img src={isGroup ? conversation.imageUrl! : otherParticipant?.user?.avatarUrl!} alt="" className="w-full h-full object-cover" />
                ) : (
              ...
              {otherParticipant && (
                <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 border-2 border-[var(--color-bg)] rounded-full ${isOnline ? "bg-[var(--color-online)] pulse-ring" : "bg-[var(--color-muted)]"}`} />
              )}
            </div>
            <div className="min-w-0">
              <div className="font-semibold text-sm text-[var(--color-text)] truncate leading-tight">{convName}</div>
              <div className="text-xs text-[var(--color-muted)] mt-0.5">
                {otherParticipant ? (
                  isOnline ? <span className="text-[var(--color-online)] font-medium">Online</span> : <span>Offline</span>
                ) : (
          ...
          <div className="flex items-center">
            <MagneticButton strength={0.15}>
              <button
                ...
                onClick={() => setShowInfo(true)} title="Conversation info" aria-label="Info"
              >
            ...
            {convMessages.length === 0 && typingList.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-12 px-4">
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                  ...
                  <div className="text-sm text-[var(--color-muted)] mb-1">No messages yet</div>
                  <div className="text-xs text-[var(--color-muted)]/60">Send a message to start the conversation</div>
                </motion.div>
              </div>
            ) : (
              <>
                ...
                <div className="py-4 space-y-0.5">
                  ...
                  {(convMessages as any[]).map((msg: any, idx: number) => {
                    ...
                    const showSender = !isSent && prevMsg?.senderId !== msg.senderId;
                    const sameAsPrevSender = !!prevMsg && prevMsg.senderId === msg.senderId;
                    const sameAsNextSender = !!nextMsg && nextMsg.senderId === msg.senderId;
                    ...
                    return (
                      <motion.div
                        ...
                        transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1], delay: Math.min(idx * 0.005, 0.15) }}
                      ...
                      >
                        ...
                        <div className={`min-w-0 max-w-[85%] sm:max-w-[70%] ${isSent ? "ms-auto" : ""}`}>
                          <MessageBubble
                            message={msg}
                            isSent={isSent}
                  ...
                  {typingList.length > 0 && (
                    ...
                    >
                      <div className="flex items-center gap-3 px-4 py-3 rounded-2xl"
                        style={{ background: "var(--color-received)", border: "1px solid var(--color-border)" }}>
                        <div className="flex gap-1">
                          {[0, 150, 300].map((delay) => (
                            <span key={delay} className="w-2 h-2 rounded-full bg-[var(--color-muted)] animate-bounce"
                              style={{ animationDelay: `${delay}ms`, animationDuration: "0.8s" }} />
                          ))}
                        </div>
                        <span className="text-xs text-[var(--color-muted)]">
                          {typingList.length === 1 ? `${typingList[0].name} is typing...` : `${typingList.length} people are typing...`}
                        </span>
                      </div>
                    </motion.div>
                  )}
        ...
        {/* Input */}
        <div className="flex-shrink-0" style={{ background: "var(--color-header)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", borderTop: "1px solid var(--color-border)" }}>
          <ChatInput
      ...
      {showInfo && conversation && <GroupInfo conversation={conversation} onClose={() => setShowInfo(false)} />}
