...
export default function GroupInfo({ conversation, onClose }: GroupInfoProps) {
  const { user } = useAuth();
  const { conversations, users, isManager, fetchConversations, deleteConversation, generateInvite, renameConversation } = useChat();
  ...
  return createPortal(
    ...
    >
      <div className="w-full max-w-lg theme-dark:bg-white/5 bg-white/90 backdrop-blur-2xl border theme-dark:border-white/10 border-gray-200 rounded-[1.5rem] shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] animate-fade-in-up max-h-[85vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b theme-dark:border-white/10 border-gray-200">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center theme-dark:text-white font-bold shrink-0 shadow-[0_0_12px_rgba(99,102,241,0.3)]">
              {(conversation.title || (conversation.isGroup ? "G" : "C")).charAt(0).toUpperCase()}
            </div>
            {editingTitle ? (
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    setRenaming(true);
                    try {
                      await renameConversation(conversation.id, titleDraft.trim());
              ...
              <div className="flex items-center gap-2 min-w-0">
                <div className="font-semibold theme-dark:text-white text-gray-900 truncate">
                  {conversation.title || (conversation.isGroup ? "Group Info" : "Conversation Info")}
                </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
          ...
          {conversation.isGroup && (() => {
            ...
            const inviteLink = groupCode ? `${baseUrl}/join?code=${groupCode}` : null;
            const copyCode = () => {
              if (groupCode) {
            ...
            return (
              <div className="theme-dark:bg-white/5 bg-gray-100 border theme-dark:border-white/10 border-gray-200 rounded-2xl p-4">
                <div className="text-xs font-semibold theme-dark:text-gray-400 text-gray-500 uppercase tracking-wider mb-3 text-center">Invitation</div>
                {groupCode ? (
                  <>
                  ...
                  <div className="text-center text-sm theme-dark:text-gray-500 text-gray-400">No code available</div>
                )}
                {canManageGroup && (
                  <button
          ...
          <div>
            ...
            <div className="theme-dark:bg-white/5 bg-gray-100 border theme-dark:border-white/10 border-gray-200 rounded-2xl overflow-hidden" style={{ maxHeight: 200, overflowY: "auto" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold theme-dark:text-white text-gray-900">
                      ...
                      {p.userId === conversation.createdById && (
                        <span className="inline-block rounded-full px-2 py-0.5 bg-blue-500/20 text-blue-400 border border-blue-500/20 ml-2" style={{ fontSize: "0.55rem", fontWeight: 600 }}>
                          Admin
                        </span>
                      )}
                    </div>
          ...
          {showAddMember && canManageGroup && (
            <div className="theme-dark:bg-white/5 bg-gray-100 border theme-dark:border-white/10 border-gray-200 rounded-2xl p-4">
              ...
              <div className="theme-dark:bg-white/5 bg-gray-100 border theme-dark:border-white/10 border-gray-200 rounded-xl" style={{ maxHeight: 160, overflowY: "auto" }}>
                {nonMembers.length === 0 ? (
                  <div className="text-center theme-dark:text-gray-500 text-gray-400 py-3 text-sm">All users are already members</div>
                ) : (
                  nonMembers.map((u) => (
                    <label
                      key={u.id}
                      className={`flex items-center gap-3 px-3 py-2 border-b theme-dark:border-white/5 border-gray-200 cursor-pointer transition-colors ${
                        selectedIds.includes(u.id) ? "bg-blue-500/10" : "theme-dark:hover:bg-white/5 hover:bg-gray-50"
                      }`}
                    >
                      <input
                        type="checkbox"
                      ...
                      <div>
                        <div className="text-sm theme-dark:text-white text-gray-900">{u.name}</div>
                        <div className="text-xs theme-dark:text-gray-500 text-gray-400">{u.role}</div>
                      </div>
                    </label>
                  ))
                )}
          ...
          {showDeleteConfirm && (
            <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-4">
              ...
              <div className="text-xs text-red-400/70 mb-3">
                This will permanently delete all messages, files, and membership records. This action cannot be undone.
              </div>
