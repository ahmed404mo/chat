...
export default function CreateChatModal({ open, onClose }: CreateChatModalProps) {
  const { user } = useAuth();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(0,0,0,0.6)" }}>
      <div className="w-full max-w-md theme-dark:bg-white/5 bg-white/90 backdrop-blur-2xl border theme-dark:border-white/10 border-gray-200 rounded-[1.5rem] shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] animate-fade-in-up">
        <div className="flex items-center justify-between px-6 py-4 border-b theme-dark:border-white/10 border-gray-200">
          <div className="font-semibold theme-dark:text-white text-gray-900">New Conversation</div>
          <button
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            <div className="theme-dark:bg-white/5 bg-gray-100 border theme-dark:border-white/10 border-gray-200 rounded-2xl overflow-hidden" style={{ maxHeight: 260, overflowY: "auto" }}>
              {otherUsers.map((u) => (
                <label
                  key={u.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div>
                      <div className="text-sm font-semibold theme-dark:text-white text-gray-900">{u.name}</div>
              ...
              {otherUsers.length === 0 && (
                <div className="text-center theme-dark:text-gray-500 text-gray-400 py-4 text-sm">No users found</div>
              )}
            </div>
          </div>
